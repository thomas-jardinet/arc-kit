...
effort: high
handoffs:
  - command: fr-marche-public
    description: Generate procurement documentation once SecNumCloud requirements are defined
    condition: "Cloud provider shortlist and qualification requirements identified"
  - command: eu-nis2
    description: Map OIV/OSE obligations to NIS2 requirements
    condition: "Entity has OIV or OSE designation"
  - command: risk
    description: Integrate SecNumCloud and extraterritorial risks into the risk register
---
...
- **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/fr-secnumcloud-template.md`
- **Then read** `${CLAUDE_PLUGIN_ROOT}/templates/_partials/RENDERING.md` and resolve the `<!-- DOC-CONTROL-HEADER -->` marker in the template before writing. `SECNUM` carries the France regime, which hard-routes to `_partials/document-control-fr.md` whatever the operator's user config says.
...
Before writing the file, read `${CLAUDE_PLUGIN_ROOT}/references/quality-checklist.md` and verify all **Common Checks** plus **SECNUM** per-type checks pass.
...
Next steps:
1. {If OIV/OSE: Run /arckit-eu:eu-nis2 for NIS2 obligation mapping}
2. Run /arckit-fr:fr-marche-public for procurement documentation
...
- **SecNumCloud became legally binding on 12 August 2026**: Before this, SecNumCloud rested on the "Cloud au Centre" doctrine (Prime Minister's circular of 5 July 2021) — a strong recommendation, with no general binding force or sanction. The arrêté du 12 août 2026 changes this: for state administrations, their operators, and the six designated GIP, holding a SecNumCloud 3.2 qualification (or an ANSSI-recognised equivalent European certification) is now a **legal requirement** for legally-sensitive data, not a best practice. Do not describe the obligation as resting on doctrine alone.
...
|----------|-----------|-----|
| Arrêté du 12 août 2026 portant approbation du référentiel d'exigences relatif aux prestataires de services d'informatique en nuage (binding legal basis, in force since 22 August 2026) | Légifrance | https://www.legifrance.gouv.fr/jorf/id/JORFTEXT000054678082 |
...
```text
/arckit-fr:fr-secnumcloud Assess SecNumCloud compliance for a health data platform at a French regional hospital group (CHR), handling données de santé, potential OSE designation

/arckit-fr:fr-secnumcloud Cloud hosting assessment for 001, ministry platform handling personal and financial data, no OIV designation

/arckit-fr:fr-secnumcloud Evaluate sovereign cloud options for a French local authority (collectivité territoriale) digital services platform, mixed-sensitivity data
```
